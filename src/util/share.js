...
// 使用方法
// 步骤1，引入：
// import * as share from '@/common/share.js'
// 步骤2，初始化：
// let shareObj = {
//     shareTitle:'',
//     shareContent: "",
...
async function commonShare(shareObj,cbFun) {
    ...
    if (sharePlatforom >= -1) {
        ...
        var url = "/local_call?local_action=share_params&arg0=" + encodeURIComponent(shareTitle) + "&arg1=" + encodeURIComponent(shareContent) + "&arg2=" + encodeURIComponent(shareIcon) + "&arg3=" + encodeURIComponent(openShareUrl) + "&arg4=" + encodeURIComponent(JSON.stringify(shareObj)) + "&arg5=" + encodeURIComponent(sharePlatforom) + "&arg6=" + encodeURIComponent(screenShotsFlag) + "&arg7=" + encodeURIComponent(imgBase64Url1) + "&arg8=" + encodeURIComponent(imgBase64Url);
...
async function addWeixinShareSpecifyInfoV2(shareObj) {
    ...
    wx.config({
        ...
        jsApiList: ['onMenuShareTimeline', 'onMenuShareAppMessage', 'onMenuShareQQ', 'onMenuShareWeibo', 'onMenuShareQZone', 'showMenuItems'] // 必填，需要使用的JS接口列表，所有JS接口列表见附录2
    });
    wx.ready(function() {
        //验证通过
        //分享到朋友圈
...
function androidShareByType(shareObj) {
    ...
    if (window.YDJSInterface && ("setMetaDataShareImgExtra" in YDJSInterface) && shareType == 1) {
        //分享图片
